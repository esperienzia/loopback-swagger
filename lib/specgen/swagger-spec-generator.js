...
module.exports = function createSwaggerObject(loopbackApplication, opts) {
  ...
  // using tags.
  const CLASS_WITHOUT_DOCS = [];
  classes
    .forEach(function (aClass) {
      if (!aClass.name) return;
      if (aClass.ctor.definition.settings.documented === false) {
        CLASS_WITHOUT_DOCS.push(aClass.name);
      }
  ...
  routes.forEach(function (route) {
    const CLASS_NAME = route.method.split('.')[0];
    if (!route.documented || CLASS_WITHOUT_DOCS.includes(CLASS_NAME)) return;
